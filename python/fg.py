...
class ImageManager(object):
    ...
    def show_image(self, parent):
        ...
        if self.cur_bitmap:
            ...
            dc.SetFont(wx.Font(35, wx.FONTFAMILY_TELETYPE, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD))
...
class MainFrame(wx.Frame):
    ...
    def OnMouseAny(self, event):
        ...
        # set the cursor to vanish in 5 seconds if there's no movement
        # and we're not paused
        if not self.image_manager.paused:
            def bleh():
                self.SetCursor(wx.StockCursor(wx.CURSOR_BLANK))
                self.mouse_timer = None
            self.mouse_timer = wx.CallLater(5000, bleh)
    ...
    def OnKeyDown(self, event):
        if not self.key_state:
            ...
            # it's prett much just space/u/d/l/r right now, I dunno what other bindings to add
            if keycode in (wx.WXK_SPACE, wx.WXK_LEFT, wx.WXK_RIGHT, wx.WXK_UP, wx.WXK_DOWN):
                self.key_state = True
